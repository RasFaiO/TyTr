...
<template>
  ...
  <Card>
    <CardHeader>
      <CardTitle>
        {{ updating ? 'Actualizar' : 'Crear' }} Servicio
      </CardTitle>
      <CardDescription>
        {{ updating ? 'Actualiza' : 'Crea' }}
        el servicio de la página principal
      </CardDescription>
    </CardHeader>
    <CardContent>
      <form>
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
          <div class="grid auto-rows-min gap-4 md:grid-cols-4">
            <div class="md:col-span-1">
              <Label for="name">Titulo de servicio</Label>
              <Input id="name" type="text" required v-model="form.name" />
              <InputError :message="form.errors.name" />
            </div>
            <div class="md:col-span-1">
              <Label for="excerpt">Descripción en tarjeta</Label>
    ...
    <CardFooter>
      <div class="w-full flex justify-end gap-4 p-4">
        ...
        <Button as-child size-sm variant="destructive">
          <Link :href="route('catService.index')">
          <Undo /> Cancelar
          </Link>
        </Button>
      </div>
